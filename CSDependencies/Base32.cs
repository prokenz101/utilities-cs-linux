        ...
        public static string? Base32ConvertMain(string[] args, bool copy, bool notif) {
            ...
            if (mode == "to") {
                ...
                return Utils.CopyNotifCheck(
                    copy, notif, new List<object>() { strToBase32, "Success!", "Message copied to clipboard." }
                );
            } else if (mode == "from") {
                try {
                    ...
                    return Utils.CopyNotifCheck(
                        copy, notif, new List<object>() { base32ToString, "Success!", $"Check your clipboard." }
                    );
                } catch (ArgumentException) {
                    return SocketJSON.SendJSON(
                        "notification", new List<object>() { "Something went wrong.", "Was that text actually Base32?" }
                    );
                }
            } else {
                return SocketJSON.SendJSON(
