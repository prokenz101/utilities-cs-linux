            ...
            switch (mode) {
                case "to":
                    try {
                        ...
                        if (Program.CurrentSettings.EscapeBase85OutputText) {
                            string? escapedBase85 = FormattableCommand.FindAndInvoke(
                                new string[] { "escape", encoded }, false, false
                            );

                            if (escapedBase85 != null) {
                                return Utils.CopyNotifCheck(
                                    copy, notif, new List<object>() { escapedBase85, "Success!", "Message copied to clipboard." }
                                );
                            }
                        }

                        return Utils.CopyNotifCheck(
                            copy, notif, new List<object>() { encoded, "Success!", "Message copied to clipboard." }
                        );
                    } catch {
                ...
                case "from":
                    try {
                        ...
                        return Utils.CopyNotifCheck(
                            copy, notif, new List<object>() { decoded, "Success!", "Message copied to clipboard." }
                        );
                    } catch {
                        return SocketJSON.SendJSON(
                            "notification", new List<object>() { "Something went wrong.", "An exception occured." }
